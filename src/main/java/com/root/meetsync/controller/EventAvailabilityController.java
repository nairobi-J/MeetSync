...
public class EventAvailabilityController {
    ...
    public List<LocalTime> generateTimeSlots(LocalTime earliest, LocalTime latest, int slotDuration) {
        ...
        System.out.println("Generating time slots from " + earliest + " to " + latest + " with duration " + slotDuration
                + " minutes");
    ...
    public String submitAvailability(@RequestParam String participantName, @RequestParam String slotIds,
            @RequestParam Long eventId, RedirectAttributes redirectAttributes) {

        List<Long> slotIdList = java.util.Arrays.stream(slotIds.split(",")).map(String::trim).filter(s -> !s.isEmpty())
                .map(Long::parseLong).collect(Collectors.toList());

        try {
            // Check if this is an update
            boolean isUpdate = participantAvailabilityRepository
                    .existsByParticipantNameAndEventSlot_Event_Id(participantName, eventId);

            availabilityService.saveAvailability(participantName, slotIdList, eventId);

            Event event = eventRepository.findById(eventId).orElseThrow(() -> new RuntimeException("Event not found"));

            String successMessage = isUpdate ? "Your availability has been updated successfully!"
            ...
            // create notification for host about new participant submission

            User host = event.getHost();
            String message = participantName + " has submitted their availability for event: " + event.getTitle() ;

            notificationService.createNotification(host, " New Participant Availability " , message,
                    Notification.NotificationType.INFO, event.getId(), "Event",
                    "/event/" + event.getShareLink() + "/overview");
    ...
    private void populateGuestModel(Model model, Event event, boolean isHost) {
        ...
        List<LocalTime> hours = generateTimeSlots(event.getEarliestTime(), event.getLatestTime(),
                event.getSlotDuration());

        // unique dates from event slots
        List<LocalDate> dates = event.getSlots().stream().map(slot -> slot.getSlotDate()).distinct().sorted()
                .collect(Collectors.toList());

        // map "date_time" -> slot
        Map<String, EventSlot> slotMap = event.getSlots().stream().collect(Collectors
                .toMap(slot -> slot.getSlotDate().toString() + "_" + slot.getStartTime().toString(), slot -> slot));
    ...
    public String showEditAvailabilityPage(@PathVariable String shareLink, @PathVariable String participantName,
